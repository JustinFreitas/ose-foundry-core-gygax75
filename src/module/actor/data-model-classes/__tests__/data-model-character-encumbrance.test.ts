...
export default ({ describe, it, expect }: QuenchMethods) => {
  ...
  describe("Basic Encumbrance", () => {
    ...
    describe("Returns current carried weight", () => {
      ...
      describe("As fully encumbered flag", () => {
        it("Encumbered over full load (2400.1)", () => {
          const enc = new EncumbranceBasic(2400, [
            createMockItem("item", 2400.1, 1, { treasure: true }),
          ]);
      ...
      it('As "over significant treasure threshold" flag', () => {
        let enc = new EncumbranceBasic(2400, [
          createMockItem("item", 400, 1, { treasure: true }),
        ]);
        expect(enc.overSignificantTreasureThreshold).to.be.false;

        enc = new EncumbranceBasic(2400, [
  ...
  describe("Detailed Encumbrance", () => {
    ...
    describe("Returns current carried weight", () => {
      ...
      describe("As fully encumbered flag", () => {
        it("Encumbered over full load (2400.1)", () => {
          const enc = new EncumbranceDetailed(2400, [
            createMockItem("item", 2400.1, 1, { treasure: true }),
          ]);
          expect(enc.encumbered).to.be.true;
        });
  ...
  describe("Complete Encumbrance", () => {
    ...
    describe("Returns current carried weight", () => {
      ...
      describe("As fully encumbered flag", () => {
        it("Encumbered over full load (2400.1)", () => {
          let enc = new EncumbranceComplete(2400, [
            createMockItem("item", 2400.1, 1, { treasure: true }),
          ]);
          expect(enc.encumbered).to.be.true;
          enc = new EncumbranceComplete(2400, [
            createMockItem("item", 2400.1, 1, { treasure: false }),
          ]);
          expect(enc.encumbered).to.be.true;
        });
        describe("Not encumbered", () => {
          it("from a partial load", () => {
            const enc = new EncumbranceComplete(2400, [
              createMockItem("item", 400, 1, { treasure: true }),
            ]);
            expect(enc.encumbered).to.be.false;
          });
          it("from a full load", () => {
            let enc = new EncumbranceComplete(2400, [
              createMockItem("item", 2400, 1, { treasure: true }),
            ]);
            expect(enc.encumbered).to.be.false;
            enc = new EncumbranceComplete(2400, [
              createMockItem("item", 2400, 1, { treasure: false }),
            ]);
            console.info(enc);
            expect(enc.encumbered).to.be.false;
          });
  ...
  describe("Item-based Encumbrance", () => {
    ...
    describe("Returns current carried weight", () => {
      ...
      describe("As fully encumbered flag", () => {
        it("Encumbered over full load (2400.1)", () => {
          let enc = new EncumbranceItemBased(2400, [
            createMockItem("item", 2400.1, 1, { treasure: true }),
          ]);
          expect(enc.encumbered).to.be.true;
          enc = new EncumbranceItemBased(2400, [
            createMockItem("item", 2400.1, 1, { treasure: false }),
          ]);
          expect(enc.encumbered).to.be.true;
        });
        describe("Not encumbered", () => {
          it("from a partial load", () => {
            const enc = new EncumbranceItemBased(2400, [
              createMockItem("item", 400, 1, { treasure: true }),
            ]);
            expect(enc.encumbered).to.be.false;
          });
